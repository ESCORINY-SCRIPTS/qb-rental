...
CreateThread(function()
  ...
  exports['qb-target']:AddBoxZone("rentvehicle", config.RentalLocations.cars.pedspawn, 1.5, 1.5, {
    ...
    options = {
      {
        type = "client",
        event = "qb-rental:client:OpenCarRental",
        icon = 'fas fa-car',
        label = 'Rent Vehicle'
      },
      {
  ...
  exports['qb-target']:AddBoxZone("rentAir", config.RentalLocations.aircrafts.pedspawn, 1.5, 1.5, {
    ...
    options = {
      {
        type = "client",
        event = "qb-rental:client:OpenAirRental",
        icon = 'fas fa-plane',
        label = 'Rent Vehicle'
      },
      {
  ...
  exports['qb-target']:AddBoxZone("rentBoat", config.RentalLocations.boat.pedspawn, 1.5, 1.5, {
    ...
    options = {
      {
        type = "client",
        event = "qb-rental:client:OpenBoatRental",
  ...
  exports['qb-target']:AddBoxZone("rentBike", config.RentalLocations.bike.pedspawn, 1.5, 1.5, {
    ...
    options = {
      {
        type = "client",
        event = "qb-rental:client:OpenBikeRental",
...
RegisterNetEvent("qb-rental:client:OpenCarRental", function()
  local CarRental = {
      {
  ...
  for k, v in pairs(config.CarRental) do
    CarRental[#CarRental + 1] = {
          ...
          txt = '$' ..v.price..'.00',
          params = {
              event = 'qb-rental:client:CarConfirm',
              args = {
                model = v.carhash,
                money = v.price,
            }
...
--submenu
RegisterNetEvent('qb-rental:client:CarConfirm', function(data)
...
AddEventHandler('qb-rental:startrental', function(data)
  local money =data.money
  local model = data.model
  ...
  QBCore.Functions.TriggerCallback("qb-rental:CheckMoney",function(money)
    if money then
      ...
      TriggerServerEvent('qb-rental:getpapers', GetVehicleNumberPlateText(vehicle), model)
    else
  ...
  if SpawnVehicle then
    ...
    StopAnimTask(PlayerPedId(), "random@atm_robbery@return_wallet_male", "return_wallet_positive_a_male", 1.0)
     ...
     StopAnimTask(PlayerPedId(), "random@atm_robbery@return_wallet_male", "return_wallet_positive_a_male", 1.0)
      ...
      TriggerServerEvent('qb-rental:takepapers')
...
RegisterNetEvent("qb-rental:client:OpenAirRental", function()
  local AirRental = {
      {
          header = 'Current Available Rentals',
          icon = 'fa-solid fa-square-info',
  ...
  for k, v in pairs(config.AirRental) do
    AirRental[#AirRental + 1] = {
          ...
          txt = '$' ..v.price..'.00',
          params = {
              event = 'qb-rental:client:AirConfirm',
              args = {
                model = v.carhash,
                money = v.price,
            }
...
--submenu
RegisterNetEvent('qb-rental:client:AirConfirm', function(data)
  local money = data.money
  local model = data.model
  exports['qb-menu']:openMenu({
      {
      ...
      },
      {
          ...
          icon = 'fa-solid fa-square-check',
          params = {
              event = 'qb-rental:client:startAirRental',
              args = {
                model = data.model,
                money = data.money,
              }
...
RegisterNetEvent('qb-rental:client:startAirRental', function(data)
  local player = PlayerPedId()
  local money = data.money
  local model = data.model
...
RegisterNetEvent("qb-rental:client:OpenBoatRental", function()
  local BoatRental = {
      {
          header = 'Current Available Rentals',
          icon = 'fa-solid fa-square-info',
  ...
  for k, v in pairs(config.BoatRental) do
    BoatRental[#BoatRental + 1] = {
          ...
          txt = '$' ..v.price..'.00',
          params = {
              event = 'qb-rental:client:BoatConfirm',
              args = {
                model = v.carhash,
                money = v.price,
            }
...
--submenu
RegisterNetEvent('qb-rental:client:BoatConfirm', function(data)
  local money = data.money
  local model = data.model
  exports['qb-menu']:openMenu({
      {
      ...
      },
      {
          ...
          icon = 'fa-solid fa-square-check',
          params = {
              event = 'qb-rental:client:startBoatRental',
              args = {
                model = data.model,
                money = data.money,
              }
...
RegisterNetEvent('qb-rental:client:startBoatRental', function(data)
  local player = PlayerPedId()
  local money = data.money
  local model = data.model
...
RegisterNetEvent("qb-rental:client:OpenBikeRental", function()
  local BikeRental = {
      {
          header = 'Current Available Rentals',
          icon = 'fa-solid fa-square-info',
  ...
  for k, v in pairs(config.BikeRental) do
    BikeRental[#BikeRental + 1] = {
          ...
          txt = '$' ..v.price..'.00',
          params = {
              event = 'qb-rental:client:BikeConfirm',
              args = {
                model = v.carhash,
                money = v.price,
            }
...
--submenu
RegisterNetEvent('qb-rental:client:BikeConfirm', function(data)
  local money = data.money
  local model = data.model
  exports['qb-menu']:openMenu({
      {
      ...
      },
      {
          ...
          icon = 'fa-solid fa-square-check',
          params = {
              event = 'qb-rental:client:startBikeRental',
              args = {
                model = data.model,
                money = data.money,
              }
...
RegisterNetEvent('qb-rental:client:startBikeRental', function(data)
  local player = PlayerPedId()
  local money = data.money
  local model = data.model
